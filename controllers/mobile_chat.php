...
class mobile_chat extends CI_Controller {
	...
	public function __construct()
     ...
     //  }
		 ...
		 function mobile_chat_user()  
		  {
			   $email=$_GET['email'];
			   $pass=$_GET['password'];
			  //exit;
			  $login=$this->gpi_model->get_client_login($email,$pass);
           // $id=$login->user_id;
			if($login) 
			{
			 ...
			 $this->gpi_model->update($data,"users",$login->user_id,"user_id"); 
			  $this->load->view("mobile_chat_user_view");
			}
		  }
		  ...
		  {
			    $data['id']=$id;
	               $this->load->view('mobile_chat_view',$data);
			 ...
			 $this->gpi_model->update($data,"chat",$res->chat_id,"chat_id");  
			  ...
			  //$this->load->view("mobile_chat_view");
		  }
		  function notify($id)  
		    {
					...
					$qry= $this->gpi_model->new_chat_updatenotification_red('chat','from_msg','to_msg',$user->user_id,$this->session->userdata('gpi_id'));
                            if(!empty($qry)){
									if($qry->notification==1){ ?>
                                      <li class="notifyuser"><a href="<?php echo base_url();?>mobile_chat/chat_view/<?php echo $user->user_id;?>"><?php if($user->profile!=""){ ?><img src="<?php echo base_url()."assets/uploads/".$user->profile; ?>" <?php } else { ?><img src="<?php echo base_url()."assets/uploads/noimage.jpg"?>" <?php } ?>  class="img-circle"> <?php echo $user->first_name;?></a></li>
                                    <?php }
									else if($user->online==1){
									  ?>
				<li class="active"><a href="<?php echo base_url();?>mobile_chat/chat_view/<?php echo $user->user_id;?>"><?php if($user->profile!=""){ ?><img src="<?php echo base_url()."assets/uploads/".$user->profile; ?>" <?php } else { ?><img src="<?php echo base_url()."assets/uploads/noimage.jpg"?>" <?php } ?>  class="img-circle"> <?php echo $user->first_name;?></a></li>
									<?php  }else{  ?>
                <li ><a href="<?php echo base_url();?>mobile_chat/chat_view/<?php echo $user->user_id;?>"><?php if($user->profile!=""){ ?><img src="<?php echo base_url()."assets/uploads/".$user->profile; ?>" <?php } else { ?><img src="<?php echo base_url()."assets/uploads/noimage.jpg"?>" <?php } ?>  class="img-circle"> <?php echo $user->first_name;?></a></li>                      
										<?php 
                                      }
							}else {
								if($user->online==1){
