 ...
 class Schedule extends CI_Controller
  {
	 ...
	 /* $html =   '<div class="col-lg-12" style="width:60%;border:1px solid #ccc;height:auto;height:260px">
	   ...
	   $this->common_model->delete_where(array('sch_id'=>$event_id),'tbl_reminder');
	   ...
	   echo "1";
		...
		echo $mysql_total->num_rows();
	     ...
	     function list_days($date_from,$date_to){
		  ...
		  $this->form_validation->set_rules('time1', 'Start Time', 'required');
		  $this->form_validation->set_rules('time2', 'End Time', 'required');
		  ...
		  if ($this->form_validation->run() == FALSE){
				   ...
				   $ddd =  $this->input->post('date_name');
				   $firstTime = $this->input->post('time1');
				   $lastTime = $this->input->post('time2');
				  
				   $start_date = $ddd." ".$firstTime;
				   $end_date = $ddd." ".$lastTime;
			...
			//	print_r($temp_cart_batch_array);die;
				...
				//@$temp_cart_batch_array[$q]['item_price']
				for($q=0; $q <count($real_time_batch_array); $q++){
					$myreal_array[] = array("date"=>$real_time_batch_array[$q]['date'],"start_time"=>$real_time_batch_array[$q]['start_time'],"end_time"=>$real_time_batch_array[$q]['end_time'],"multiple_delete"=>$real_time_batch_array[$q]['multiple_delete'],"start_date_unix"=>$real_time_batch_array[$q]['start_date_unix'],"end_date_unix"=>$real_time_batch_array[$q]['end_date_unix'],"sch_id"=>$event_id,'option_type'=>$optType);
					
					$myreminderArray[] = array('item_name'=>$insert_array['sch_name'],'sch_date'=>$real_time_batch_array[$q]['start_date_unix'],'user_id'=>$adminId,'email'=>$emailUser,'item_price'=>@$option_price,'remember_time'=>$rem,'sch_id'=>$event_id."_1",'notes'=>$insert_array['note']);
			...
			$dddd =  $this->input->post('date_name');
			$firstTime = $this->input->post('time1');
			$lastTime = $this->input->post('time2');
			$sss_date = $dddd." ".$firstTime;
			$eee_date = $dddd." ".$lastTime;
				...
				print_r($existingRecArray);die;
					   ...
					   if($this->input->post('event_type') == 1){
							...
							if($recurring_opt == "daily"){
								$ss_abc = $this->input->post('date_name');
								...
								$sdaterecrring = strtotime($this->input->post('date_name'));
									...
									//print_r($return);
									 ...
									 if(!empty($return[0])){
										$existingRecArray[] = $return[0];
										   ...
										   $myreminderArray_recurring = array('item_name'=>$insert_array['sch_name'],'sch_date'=>strtotime($validatedateArray[$j][$q]." ".$ss_timeDb),'user_id'=>$adminId,'email'=>$emailUser,'item_price'=>$option_price,'remember_time'=>$rem,'sch_id'=>$event_id."_1",'notes'=>$insert_array['note']);
										   
										   $this->common_model->insert_array('tbl_reminder',$myreminderArray_recurring);
										   $UserSendng = explode(",",$receiverEmail);
										   
										   
										   for($qs=0; $qs<count($UserSendng); $qs++){
											    /*
												$myUserID = "";
												$mysqlUserArr =  $this->common_model->select_where("user_id","users",array("email"=>$UserSendng[$qs]));
												if($mysqlUserArr->num_rows() >0){
													$mysqlUserArr = $mysqlUserArr->row_array();
													$myUserID = $mysqlUserArr['user_id'];
												}else{
													$myUserID = $this->common_model->select_single_field("guest_user_id","guest_user",array("email"=>$UserSendng[$qs]));	
												}*/
												
										  										   
											   $myschedlue_crtArray = array(
												   "sch_name"=>$validatedateArray[$j][$q]." ".$ss_timeDb."_".$insert_array['sch_name'],
												   "sch_id"=>$event_id."_admin",
												   "sch_price"=>$option_price,
												   "option_id"=>$package_opt,
												   "sch_notes"=>$this->input->post('note'),
												   "lock_item"=>"free",
												   "user_id"=> $my_user_IdsArray[$qs],
												   "status"=>"inactive",
												   "user_email"=>$UserSendng[$qs],
											   );
											   
											   $this->common_model->insert_array('tbl_schedule_cart',$myschedlue_crtArray);
										   }
										   ...
										   $temp_cart_batch_array_temp = array();
										   $dd = $validatedateArray[$j][$q]." ".$ss_timeDb;
										   $startTime = strtotime($dd);
										   $chap_1 = 30;
										   $chap = 0;
										   $end_date = $validatedateArray[$j][$q]." ".$ee_timeDb;
											...
											while(strtotime($dd) < strtotime($end_date)){
												...
												if(!in_array($chap_time,$myArr)){
												 ...
												 //$insertingArrTemp['item_id']		  = "admin_".$package_opt.$inser_id;
												 $insertingArrTempOne['item_id']		  = "admin_".$package_opt;
												 $insertingArrTempOne['type']    		  = "admin";
												 $insertingArrTempOne['item_name']		  = $dd."_".$option_name;
												 $insertingArrTempOne['item_type'] 	 	  = "schedule";
												 $insertingArrTempOne['item_qty'] 		  = 1;
												 $insertingArrTempOne['item_price'] 	  = $option_price;
												 $insertingArrTempOne['item_discount']    = 0;
												 $insertingArrTempOne['type']   		  = 'admin';
												 $temp_cart_batch_array_temp[] = array('item_id'=>$insertingArrTempOne['item_id'],'type'=>$insertingArrTempOne['type'],'item_name'=>$insertingArrTempOne['item_name'],'item_type'=>$insertingArrTempOne['item_qty'],'item_price'=>$insertingArrTempOne['item_price'],'item_discount'=>$insertingArrTempOne['item_discount'],'type'=> $insertingArrTempOne['type']);
											//	 $inser_id = $this->common_model->insert_array('tbl_temp_cart_items',$insertingArrTemp);
											}
											
											//echo "<pre>";
											
											//print_r($temp_cart_batch_array_temp);
											
											if(!empty($temp_cart_batch_array_temp[0])){
												$this->db->insert_batch('tbl_temp_cart_items', $temp_cart_batch_array_temp); 
											}
	  ...
	  public function existing_dbDate($fDateTime,$eDateTime,$event_id=""){
		  $myDateArray = array();
		   ...
		   $mysqlAlldatesEnd = $this->db->query("SELECT * FROM tbl_real_time WHERE ".$eDateTime." BETWEEN start_date_unix AND end_date_unix");
			if(($mysqlAlldatesStart->num_rows() >0) || ($mysqlAlldatesEnd->num_rows() >0)){
				 $myDateArray[] =  date("F j, Y",$fDateTime);
			}
		    return $myDateArray;
