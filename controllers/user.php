<?php
  class user extends CI_Controller
  {
   ...
   public function update_notes(){
		...
		$main_update = $this->input->post('main_update');
		if($main_update !=""){
			$this->common_model->update_array(array('sch_id'=>$sch_id),'tbl_schedule_cart',array('sch_notes'=>$notes));	
		}else{
			$this->common_model->update_array(array('sch_id'=>$sch_id),'tbl_schedule_cart',array('update_sch_notes'=>$notes));
		}
		echo "1";
	}
   ...
   public function notify_url($user_id,$upgradeId,$frequancy=0){
    ...
    public function update_temprary_cartItems(){
		   ...
		   if($new_item_price > $old_item_price){
			...
			$ser_id = $this->common_model->select_single_field('id','tbl_service',array('ser_name'=>$new_dropdownValue));
			
			$insertSchedule['updated_sch_name']   = $new_itemsArr."_".$new_dropdownValue;
			$insertSchedule['updated_sch_id']	  = $new_itemsArr."_".$ser_id;
			...
			$insertSchedule['update_varify_start_date'] = $this->input->post('varify_start_date');
			$insertSchedule['update_varify_end_date'] = $this->input->post('varify_end_date');
   ...
   function getName($id){
	...
	//	$arrayLevel = array_column($electID,'level_id');
	 ...
	 function upcommingclass_view()
	  ...
	  function show_sub_folder($main_folder,$sub_folder_id)
	 	 {
				...
				$data['id'] =	$sub_folder_id; 
				$data['content'] = 'sub-folder/sub_folder_view';
				$this->load->view('layout/layoutuser',$data);
	  }
	  ...
	  {	   
		   ...
		   $mysql_subfolders = 	$this->common_model->select_where("*","resource_folder",array("parent_id"=>$id,"type"=>"sub"));
		 ...
		 // print_r($this->session->all_userdata());
		  ...
		  if($type !='pkg'){
			 $this->common_model->update_array(array('user_id'=>$user_id,'item_id'=>$item_id,'item_type'=>$type),'membership_access',array('lock_item'=>'yes'));
			   ...
			   $pkg_id = $this->common_model->select_single_field('pkg_id','tbl_package_access',array('user_id'=>$user_id,'access_id'=>$item_id,'type'=>$type));
			   $this->common_model->delete_where(array('item_id'=>$pkg_id),'tbl_temp_cart_items');	  
		  }
		  
		   $mysqlSch =$this->common_model->select_where("*","tbl_schedule_cart",array("sch_id"=>$item_id));
		   
		  if($mysqlSch->num_rows() >0){
			 $mysqlSch =  $mysqlSch->row_array();
			 if($mysqlSch['updated_sch_id'] !=""){
				...
				$this->common_model->update_array(array('sch_id'=>$item_id),'tbl_schedule_cart',$udateRec);
		 	 }
		  }
		  
		 $item_id =  str_replace("%20"," ",$item_id);
		 $this->common_model->delete_where(array('cart_id'=>$cart_id),'tbl_cart');
		 $this->common_model->delete_where(array('item_id'=>$item_id),'tbl_temp_cart_items');	  
	  ...
	  function clear_cart($item_id='',$user_id='',$type=''){
		  ...
		  $mysql_scheduleIds = $this->common_model->select_where('item_id','tbl_cart',array('item_type'=>'schedule'));
		  
		  if($mysql_scheduleIds->num_rows() >0){
			  foreach($mysql_scheduleIds->result_array() as $scheduleObj){
				  $mysqlSch =$this->common_model->select_where("*","tbl_schedule_cart",array("sch_id"=>$scheduleObj['item_id']));
				  ...
				  if($mysqlSch->num_rows() >0){
					 ...
					 if($mysqlSch['updated_sch_id'] !=""){
						...
						$udateRec['status']= "active";
						$udateRec['lock_item']= "free";
						$this->common_model->update_array(array('sch_id'=>$scheduleObj['item_id']),'tbl_schedule_cart',$udateRec);
					  }else{
						$this->common_model->delete_where(array('sch_id'=>$scheduleObj['item_id']),'tbl_schedule_cart');	  
					 }
	  ...
	  function cart_paypal_success($user_id=''){
		  ...
		  if($mysqlCarts->num_rows() >0){
			...
			//	$this->session->unset_userdata('ticket_qty');
				//$this->session->unset_userdata("myTicket_id");
				  ...
				  }else if($mysqlObj['item_type'] == "schedule"){
					  //echo $mysqlObj['item_id'];die;
					  $single_detail1 =   $this->common_model->select_where('*','tbl_schedule_cart',array('sch_id'=>$mysqlObj['item_id']));
					  $s=0;
					  $item_name = "";
					 
					  if($single_detail1->num_rows() >0){
					      $single_detail =   $single_detail1->row_array();
						  $this->common_model->update_array(array('sch_id'=>$mysqlObj['item_id'],'user_id'=>$user_id),'tbl_schedule_cart',array('status'=>'active'));
						 // echo "first Case";
						//	print_r($single_detail);die;
						
						//in case of update the scheule on new time slot..
						 if($single_detail['updated_sch_price']!=""){
							$s=1;
							...
							$schNameExp =  explode("_",$single_detail['updated_sch_name']);
							$item_name = $schNameExp[1]." (".$schNameExp[0]." )";
							$discount  = 0;
							$price = $single_detail['updated_sch_price'];
							$pp_price = 	$price;
							
							
							$this->common_model->update_array(array('sch_id'=>$mysqlObj['item_id']),'tbl_schedule_cart',$udateArray);
							
							$emailUser = $this->common_model->select_single_field('email','users',array('user_id'=>$user_id));
							
							$this->common_model->insert_array('tbl_reminder',array('item_name'=>$item_name,'sch_date'=>strtotime($schNameExp[0]),'end_date'=>0,'user_id'=>$user_id,'sch_id'=>$single_detail['updated_sch_id'],'email'=>$emailUser,'item_price'=>$price,'remember_time'=>'1day,4hours','notes'=>$udateArray['sch_notes']));
							
						 }else{
							 $schNameExp =  explode("_",$single_detail['sch_name']);
							 $item_name = $schNameExp[1]." (".$schNameExp[0]." )";
							 $emailUser = $this->common_model->select_single_field('email','users',array('user_id'=>$user_id));
							 $discount  = 0;
							 $price = $single_detail['sch_price'];
							 $pp_price = 	$price;
							 
							 //  echo "second case.<br />";
						     // print_r($single_detail);die;
							 
							 $this->common_model->insert_array('tbl_reminder',array('item_name'=>$item_name,'sch_date'=>strtotime($schNameExp[0]),'end_date'=>0,'user_id'=>$user_id,'sch_id'=>$single_detail['sch_id'],'email'=>$emailUser,'item_price'=>$price,'remember_time'=>'1day,4hours','notes'=>$single_detail['sch_notes']));
								...
								if($s == 0){
									$mysql_adminAppointment = $this->common_model->select_where('*','tbl_schedule',array('id'=>$mysqlObj['item_id']));
									
									if($mysql_adminAppointment->num_rows() >0){
										
										$mysql_adminAppointment = $mysql_adminAppointment->row_array();
										//print_r($mysql_adminAppointment);
										//echo "fOurath dcase";die;
										
										$date = $mysql_adminAppointment['start_date'];
										$end_DATE_TIME = $mysql_adminAppointment['end_date'];
										$service = $mysql_adminAppointment['sch_name'];
										$item_name = $date."_".$service;
										$shedultCartArr['sch_name']	 =  $item_name;
										$shedultCartArr['sch_id']	 =  $mysql_adminAppointment['id']."_".'admin';
										$shedultCartArr['sch_price']	 =  $mysql_adminAppointment['event_price'];
										$shedultCartArr['sch_discount']	 =  0;
										$shedultCartArr['option_id'] 	 =  $mysql_adminAppointment['option_id'];
										$shedultCartArr['user_id']	 	 =  $user_id;
										$pp_price = $mysql_adminAppointment['event_price'];
										$discount  = 0;
										$price 	= $pp_price - $discount;
										$dis = $discount;
										$price = $price;
										
										 $this->common_model->update_array(array('sch_id'=>$shedultCartArr['sch_id'],'user_id'=>$user_id),'tbl_schedule_cart',array('status'=>'active'));
										
										
										$this->common_model->insert_array('tbl_schedule_cart',$shedultCartArr);	
									//	echo $this->db->last_query();die;
										$remember_time = $this->common_model->select_single_field('remember_time','tbl_schedule',array('id'=>$mysql_adminAppointment['id']));
										
										$this->common_model->update_array(array('sch_id'=>$mysql_adminAppointment['id']),'tbl_real_time',array('type'=>1));
										
										$emailUser = $this->common_model->select_single_field('email','users',array('user_id'=>$user_id));
																
										$this->common_model->insert_array('tbl_reminder',array('item_name'=>$item_name,'sch_date'=>strtotime($date),'end_date'=>strtotime($end_DATE_TIME),'user_id'=>$user_id,'sch_id'=>$mysql_adminAppointment['id'],'email'=>$emailUser,'item_price'=>$price,'remember_time'=>$remember_time,'notes'=>$mysql_adminAppointment['note']));
										
									
										
										$mysql_recurring = $this->common_model->select_where('*','tbl_schedule_recurring',array('sch_id'=>$mysql_adminAppointment['id']));
										$mysql_batch_recurring = array();
										
										if($mysql_recurring->num_rows() >0){
											foreach($mysql_recurring->result_array() as $recurrObj){
													$mysql_batch_recurring[] = array('item_name'=>$item_name,'sch_date'=>$recurrObj['recurring_date'],'end_date'=>strtotime($end_DATE_TIME),'user_id'=>$user_id,'sch_id'=>$mysql_adminAppointment['id'],'email'=>$emailUser,'item_price'=>$price,'remember_time'=>$remember_time,'notes'=>$mysql_adminAppointment['note']);
											}	
										}
										...
										$schArray = $this->common_model->select_where('*','tbl_temp_cart_items',array('item_id'=>$mysqlObj['item_id']));
										$schArray = $schArray->row_array();
										$schNameExp =  explode("_",$schArray['item_name']);
										if(!empty($schNameExp)){
											$item_name = $schNameExp[1]." (".$schNameExp[0]." )";
										}
										$pp_price = $schArray['item_price'];
										$discount  = 0;
										$price 	= $pp_price  - $discount;
										$dis = $discount;
										$price = $price;
										
										$shedultArr['sch_name']	 =  $schArray['item_name'];
										$shedultArr['sch_id']	 =  $schArray['item_id'];
										$shedultArr['sch_price']	 =  $schArray['item_price'];
										$shedultArr['sch_discount']	 =  0;
										$itemFull = explode("_",$schArray['item_name']);
										...
										$shedultArr['user_id']	 = $user_id;
										$shedultArr['sch_notes'] = $schArray['notes'];
										
										$shedultArr['date_varify_start']	 = $schArray['date_varify_start'];
										$shedultArr['date_varify_end'] 		 = $schArray['date_varify_end'];
										
										$emailUser = $this->common_model->select_single_field('email','users',array('user_id'=>$user_id));
										$this->common_model->insert_array('tbl_reminder',array('item_name'=>$item_name,'sch_date'=>strtotime($schNameExp[0]),'end_date'=>0,'user_id'=>$user_id,'sch_id'=>$schArray['sch_id'],'email'=>$emailUser,'item_price'=>$price,'remember_time'=>'1day,4hours','notes'=>$schArray['notes']));
										
										
										//$this->common_model->delete_where(array('sch_id'=>$schArray['item_id']),'tbl_schedule_cart');
										if(!empty($schArray['item_name'])){
											$this->common_model->insert_array('tbl_schedule_cart',$shedultArr);	
										}
									}
								}
						 }
			  ...
			  $string .=  "<table align='center' style='width:90%;border: 1px solid;' cellpadding='10'>
					...
					$msgHtml = '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
							...
							$this->email->send();
							 ...
							 if(mail($useremail,'Order Detail',$data,$headers)){}
		...
		$cartItemsObj =  $this->common_model->select_where('*','tbl_cart',array('user_id'=>$user_id));
			if($cartItemsObj->num_rows() >0){
			 ...
			 $this->common_model->delete_where(array('user_id'=>$user_id),'tbl_cart');
			 $login_session_id = $this->session->userdata('login_session_id');
			 $this->common_model->delete_where(array('session_id'=>$login_session_id,'type <>'=>'admin'),'tbl_temp_cart_items');
			...
			//   print_r($mysqlCarts->result_array());die;
			  foreach($mysqlCarts->result_array() as $mysqlObj){
				  ...
				  if($mysqlObj['item_type'] == "res"){
					 ...
					 $mysql_schedule = $this->common_model->select_where('*','tbl_temp_cart_items',array('item_id'=>$mysqlObj['item_id']));
					 if($mysql_schedule->num_rows() >0){
						 $mysql_schedule = $mysql_schedule->row_array();
						 
						 $price 	= $mysql_schedule['item_price'];
						 
						 if(($price  < 1)&& ($mysqlCarts->num_rows() ==1)){
							
							 header("Location: ".base_url()."user/cart_paypal_success"."/".$user_id);
							 break;
							//$this->cart_paypal_success($user_id);
						 }
						
						 $discount = 0;
						 $price = $price - $discount;
						 $item_naming =  $mysql_schedule['item_name'];
						 $itemSingle = explode("_",$item_naming);
						 $item_name = $itemSingle[0]. " (".$itemSingle[1]." )";
					
					 }else{
						 
						  $mysql_scheduleCart = $this->common_model->select_where('*','tbl_cart',array('item_id'=>$mysqlObj['item_id']));
						 if($mysql_scheduleCart->num_rows() >0){
							$mysql_scheduleCart = $mysql_scheduleCart->row_array();
							$mysql_scheduleItem = $this->common_model->select_where("*","tbl_schedule",array("id"=>$mysql_scheduleCart['item_id']));
							$mysql_scheduleItem = $mysql_scheduleItem->row_array();
							$price 	= $mysql_scheduleItem['event_price'];
						 	$discount = 0;
							$price = $price - $discount;
							$item_name = $mysql_scheduleItem['start_date']. " (".$mysql_scheduleItem['sch_name']." )";
						 }
	...
	function show_notes(){
		...
		if($flag == 0){
			echo $this->common_model->select_single_field('notes','tbl_temp_cart_items',array('id'=>$sch_id));
		}else{
			echo $this->common_model->select_single_field('sch_notes','tbl_schedule_cart',array('id'=>$sch_id));
		}
	}
	
	function update_notes_cart(){
		$notes = $this->input->post('notes');	
		$id = $this->input->post('id');	
		$flag = $this->input->post('flag');	
		
		if($flag == 0){
			$this->common_model->update_array(array('id'=>$id),'tbl_temp_cart_items',array('notes'=>$notes));
		}else{
			$this->common_model->update_array(array('id'=>$id),'tbl_schedule_cart',array('sch_notes'=>$notes));
		}
